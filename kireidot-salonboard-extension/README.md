...
## v0.0.2: 予約同期くんとの自動連携
拡張は2つの使い方ができる:
1. **手動**（popup）: SalonBoardを開いて拡張アイコン→画像URL入力→実行。
2. **自動**（予約同期くん連携）: 予約同期くんの「スタイル」ページで「スタイル投稿(Chrome拡張)」を押すと、
   - 予約同期くんが `127.0.0.1:32178` でジョブを作り、普段使いChromeで styleEdit を開く。
   - 拡張(background.js)が `127.0.0.1:32178/jobs/next` をポーリングしてジョブを取得。
   - SalonBoardタブの content.js に送って FRONT 画像を自動アップロード。
   - 結果を `/jobs/:jobId/complete` に返し、予約同期くんの画面に状態表示。
   - ※予約同期くん(v0.2.137以降)が起動している必要があります。
