...
### ✅回収済み: 死んだ旧Chromeプロファイル 5.1GB(2026-08-09実施)

capture は掃除タイマーで**約1.6GBで頭打ち**になる(225MB/日 × 7日)。しかしメイン箱の使用19GBのうち capture は1.8GBに過ぎず、**12GBはChromeプロファイル**が占める。

2026-08-09の実測でその内訳が判明した:

| プロファイル種別 | 個数 | サイズ | 状態 |
|---|---|---|---|
| `account-<hash>`(現行) | **16** | 6.9GB | 稼働中。**消してはいけない** |
| UUID形式(旧shop単位)/ `.bak-20260703` | **18** | **5.1GB** | **7/22以降まったく未使用** |

**旧プロファイルが参照されないことの根拠**:

1. `worker.ts:3266` の移行ロジックは `if (!existsSync(userDataDir) && legacyDir && existsSync(legacyDir))` — **アカウントプロファイルが存在しない場合のみ**旧プロファイルをコピーする
2. DB上の稼働中SBアカウント数(グループはlogin_id集約)= **16**、箱上の `account-` プロファイル数 = **16** で**完全一致**。全アカウントが移行済み
3. 旧プロファイルの最終更新は 2026-07-02 / 07-22 で以降更新なし

したがって旧18個を削除しても `existsSync(userDataDir)` が真であり続けるため、**再ログインは一切発生しない**。

**実施結果(2026-08-09)**: `scripts/prune-legacy-chrome-profiles.sh --apply` で18個5.1GBを削除。**19GB → 14GB(38% → 28%)**。account-プロファイル16個は保持、コンテナ再起動なし。削除直後に `push_coupon` が12件成功しており**既存セッションは生存**を実証。削除一覧は箱上 `/opt/kireidot/pruned-legacy-profiles.log` に記録。移行は完了済みなので**二度と作られない**。

スクリプトは冪等で、既定はドライラン。`account-` 以外のみを対象とし、`account-` の個数が指定数未満の箱では何もしない安全ゲート付き。

**⚠️ 現行の `account-` プロファイルは消してはいけない。** Akamaiの `_abck` 信頼を蓄積しており、消すと全店コールドログイン→CAPTCHA/throttleの波になる。

参考: `BrowserMetrics` は7月分44MB / 8月分1084MBで古い分は減っており、ある程度自己回収されている。`Cache` は6月のファイルが残存し完全にはローテしない。ただし現行16プロファイルで6.9GBなので、旧プロファイルを片付ければ当面ディスクは逼迫しない。
