...
async function pushShiftsViaForm(page, payload, opts = {}) {
  ...
  //   (2026-07-26 代々木上原で実害: パターン復旧の全日再割当がSBの予定を一掃)。
  //   日別モーダルは既存の予定行を載せたまま保存するため、パターンだけ変えても
  //   予定が残る。休日(off)化は従来どおり一括入力(休日設定に予定行は無い)。
  const applyPatternDay = async (plan, ymd) => {
    const cellSel = `#${plan.staffExt}_${ymd}`;
    ...
    if (!meta || (meta.date && meta.date !== ymd) || (meta.staff && meta.staff !== plan.staffExt)) {
...
async function changeBookingViaForm(page, payload, opts = {}) {
  ...
  // 3.1) KDで割り当て済みの設備を予約変更フォームにも明示的に反映する。
  // 新規登録では設備を設定していたが、変更フローは担当・日時・所要時間しか更新して
  // いなかった。そのため、SB由来予約の日時変更時に既存設備が空へ戻っても
  // 「変更成功」として扱われていた（2026-07-25 中目黒 YG91741483）。
  ...
  if (staffExt || (!looksDone && !confirmClicked && !nativeDialogAccepted)) {
    ...
    for (const path of candidates) {
      ...
      persistedState = await page.evaluate(({ wantHour, wantMinute, wantHhmm, wantDuration, wantStaffExt }) => {
        ...
        const staffValues = Array.from(document.querySelectorAll([
          'select#salonStaffList',
          'input#staffId',
          'input[name="staffId"]',
          'select[name="salonStaffList"]',
          ...
          .map((el) => String(el.value || '').trim().toUpperCase())
          .filter(Boolean);
        const staffOk = !wantStaffExt
          || staffValues.includes(String(wantStaffExt).trim().toUpperCase());
        return {
          estheticOk,
          hairOk,
          staffOk,
        ...
        wantHhmm: hhmm,
        wantDuration: dMin,
        wantStaffExt: staffExt,
      }).catch(() => null);
      persisted = !!persistedState?.staffOk
        && !!(persistedState?.estheticOk || persistedState?.hairOk);
      break;
    }
    if (!persisted) {
